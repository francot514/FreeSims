...
namespace TSOVille.Code.UI.Screens
{
    public class LoginScreen : GameScreen
    {
        ...
        public LoginScreen()
        {
            ...
            if (GlobalSettings.Default.LastUser != "")
                LoginDialog.Username = GlobalSettings.Default.LastUser;

            if (GlobalSettings.Default.LastPass != "")
                LoginDialog.Password = GlobalSettings.Default.LastPass;
        ...
        private void Controller_OnLoginProgress(TSOVille.Network.Events.ProgressEvent e)
        {
            var stage = e.Done;
        ...
        private void Controller_OnLoginStatus(TSOVille.Network.Events.LoginEvent e)
        {
            m_InLogin = false;
            if (e.Success)
            {
        ...
        public void Login()
        {
             ...
             if (LoginDialog.Username != "" && LoginDialog.Password != "")
           {
            ...
            Options.Message = "Succesfully logged in!";
            Options.Title = "User connection";
            UI.Framework.UIScreen.ShowAlert(Options, true);
...

           {
            ...
            //Controller_OnLoginProgress(new TSOVille.Network.Events.ProgressEvent(TSOVille.Events.EventCodes.PROGRESS_UPDATE) { Done = 1 });
            //NetworkFacade.Controller.InitialConnect(LoginDialog.Username.ToUpper(), LoginDialog.Password.ToUpper());
        }

        /// <summary>
        /// Called by register button click in UILoginDialog
        /// </summary>
        public void Register()
        {
            ...
            if (LoginDialog.Username != "" && LoginDialog.Password != "")
           {
            ...
            Options.Message = "The user was registered!";
            Options.Title = "User registration";
            UI.Framework.UIScreen.ShowAlert(Options, true);
           }
            else
           {
            Options.Message = "Cannot register user, missing fields!";
            Options.Title = "User registration";
            UI.Framework.UIScreen.ShowAlert(Options, true);
