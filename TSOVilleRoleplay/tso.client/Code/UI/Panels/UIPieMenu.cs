    ...
    public class UIPieMenu : UIContainer
    {
        ...
        private void PieButtonClick(UIElement button)
        {
            ...
            else
            {
                ...
                {
                    var newGoto = m_Parent.vm.Context.CreateObjectInstance(0x000007C4, new LotTilePos(m_Obj.Position), Direction.NORTH).Objects[0];
                    newGoto.PushUserInteraction(action.ID, m_Caller, m_Parent.vm.Context);
                }
                else
                {
